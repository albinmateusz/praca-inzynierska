...
# Model Forms.
from .forms import PostAdsForm, EditAdsForm
...
def post_ads(request):
    category_list = Category.objects.values_list('category_name', flat=True).distinct().order_by("category_name")
    condition_list = Ads.objects.values_list('condition', flat=True).distinct().order_by("condition")
    if request.method == 'POST':
        ...
        # Create the ad
        ads = Ads.objects.create(author=request.user.author, title=title, description=description, price=price,
                                 category=c, condition=condition, state=s, city=ci, brand=brand, phone=phone, )
...
def ads_search(request):
    ...
    # if title:
    #     ads_search_result = Ads.objects.filter(title__contains=title)
    # elif state:
    ...
    if state == "":
        ...
        ads_search_result = Ads.objects.filter(title__contains=title).filter(state__state_name=state)
...
def ads_edit(request, pk):
    instance = Ads.objects.get(id=pk)
    if request.method == 'POST':

        edit_form = EditAdsForm(request.POST, instance=instance)
        if edit_form.is_valid():
            edit_form.save()
            messages.success(request, f"Zaktualizowano pomyslnie")
            return redirect('profile-ads')
    else:
        edit_form = EditAdsForm(instance=instance)

    context = {
        'edit_form': edit_form,
    }
    return render(request, 'ads/ads-edit.html', context)


def ads_success(request):
    return render(request, 'post-ads/ads_success.html')
